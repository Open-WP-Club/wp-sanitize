 ...
 * Description:             Sanitizes data for staging environments with various options and batching, including WooCommerce support
 * Version:                 1.1.1
 * Author:                  Open WP Club
...
<?php
    ...
    public function page_init()
    {
        ...
        if ($this->is_woocommerce_active()) {
            ...
            add_settings_field(
                'sanitize_wc_reviews',
                'Sanitize WooCommerce Product Reviews',
                array($this, 'checkbox_callback'),
                'wp-data-sanitizer-admin',
                'wp_data_sanitizer_setting_section',
                array('sanitize_wc_reviews')
            );
            add_settings_field(
                'sanitize_wc_coupons',
                'Sanitize WooCommerce Coupons',
                array($this, 'checkbox_callback'),
                'wp-data-sanitizer-admin',
                'wp_data_sanitizer_setting_section',
                array('sanitize_wc_coupons')
            );
            if (class_exists('WC_Subscriptions')) {
                add_settings_field(
                    'sanitize_wc_subscriptions',
                    'Sanitize WooCommerce Subscriptions',
                    array($this, 'checkbox_callback'),
                    'wp-data-sanitizer-admin',
                    'wp_data_sanitizer_setting_section',
                    array('sanitize_wc_subscriptions')
                );
            }
        }
    }
    ...
    public function sanitize($input)
    {
        ...
        $new_input['sanitize_wc_reviews'] = isset($input['sanitize_wc_reviews']) ? true : false;
        $new_input['sanitize_wc_coupons'] = isset($input['sanitize_wc_coupons']) ? true : false;
        $new_input['sanitize_wc_subscriptions'] = isset($input['sanitize_wc_subscriptions']) ? true : false;
        return $new_input;
    }
    ...
    public function enqueue_admin_scripts($hook)
    {
        ...
        wp_enqueue_script('wp-data-sanitizer-admin-js', plugins_url('assets/js/admin.js', __FILE__), array('jquery'), '1.2.0', true);
        wp_localize_script('wp-data-sanitizer-admin-js', 'wpDataSanitizer', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('wp_data_sanitizer_nonce')
        ));
        wp_enqueue_style('wp-data-sanitizer-admin-css', plugins_url('assets/css/admin.css', __FILE__), array(), '1.2.0');
    }
    ...
    public function sanitize_batch()
    {
        ...
        try {
            ...
            if ($this->is_woocommerce_active()) {
                if ($options['sanitize_wc_orders']) {
                    $this->sanitize_wc_orders_batch($offset, $batch_size, $total, $processed, $error_log);
                }
                if ($options['sanitize_wc_reviews']) {
                    $this->sanitize_wc_reviews_batch($offset, $batch_size, $total, $processed, $error_log);
                }
                if ($options['sanitize_wc_coupons']) {
                    $this->sanitize_wc_coupons_batch($offset, $batch_size, $total, $processed, $error_log);
                }
                if ($options['sanitize_wc_subscriptions'] && class_exists('WC_Subscriptions')) {
                    $this->sanitize_wc_subscriptions_batch($offset, $batch_size, $total, $processed, $error_log);
                }
            }
  ...
  private function sanitize_wc_orders_batch($offset, $batch_size, &$total, &$processed, &$error_log)
  {
    global $wpdb;
  ...
  private function sanitize_wc_reviews_batch($offset, $batch_size, &$total, &$processed, &$error_log)
  {
    $args = array(
      'post_type' => 'product',
      'status' => 'approve',
    ...
    foreach ($reviews as $review) {
      $sanitized_content = 'Sanitized review for product ID: ' . $review->comment_post_ID;
      $result = wp_update_comment(array(
        'comment_ID' => $review->comment_ID,
        'comment_content' => $sanitized_content,
        'comment_author' => 'Anonymous',
        'comment_author_email' => 'anonymous' . $review->comment_ID . '@example.com',
      ));
      if ($result === false) {
        $error_log[] = "Failed to update review ID: " . $review->comment_ID;
      }
      $processed++;
    }
  }

  private function sanitize_wc_coupons_batch($offset, $batch_size, &$total, &$processed, &$error_log)
  {
    $args = array(
    ...
    foreach ($coupons as $coupon) {
      $sanitized_code = 'SANITIZED_' . $coupon->ID;
      $result = wp_update_post(array(
        'ID' => $coupon->ID,
        'post_title' => $sanitized_code,
        'post_name' => sanitize_title($sanitized_code),
      ));
      if ($result === 0) {
        $error_log[] = "Failed to update coupon ID: " . $coupon->ID;
      } else {
        update_post_meta($coupon->ID, 'coupon_amount', '0');
        update_post_meta($coupon->ID, 'customer_email', array());
      }
      $processed++;
    }
  }

  private function sanitize_wc_subscriptions_batch($offset, $batch_size, &$total, &$processed, &$error_log)
  {
    if (!class_exists('WC_Subscriptions')) {
      return;
    }
    ...
    $total += wp_count_posts('shop_subscription')->publish;
    $subscriptions = get_posts($args);

    foreach ($subscriptions as $subscription) {
      $wc_subscription = wcs_get_subscription($subscription->ID);
      if ($wc_subscription) {
        $wc_subscription->set_billing_first_name('John');
        $wc_subscription->set_billing_last_name('Doe');
        $wc_subscription->set_billing_company('Example Company');
        $wc_subscription->set_billing_address_1('123 Main St');
        $wc_subscription->set_billing_city('Anytown');
        $wc_subscription->set_billing_state('CA');
        $wc_subscription->set_billing_postcode('12345');
        $wc_subscription->set_billing_country('US');
        $wc_subscription->set_billing_email('john.doe' . $subscription->ID . '@example.com');
        $wc_subscription->set_billing_phone('555-123-4567');
        $result = $wc_subscription->save();
        if (is_wp_error($result)) {
          $error_log[] = "Failed to update subscription ID: " . $subscription->ID . ". Error: " . $result->get_error_message();
        }
      } else {
        $error_log[] = "Failed to get subscription object for ID: " . $subscription->ID;
      }
      $processed++;
    }
  }
